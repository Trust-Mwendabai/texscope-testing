...
import { BriefcaseIcon, GraduationCapIcon, TrophyIcon, TargetIcon } from 'lucide-react';
import { Badge } from '@/components/ui/badge';

export interface TimelineWorkItem {
   type: 'project' | 'training' | 'achievement' | 'milestone';
   user: string;
   content: string;
   project?: string;
   time?: string;
}
export type TimelineItem = TimelineWorkItem;
...
const iconForType = (type: TimelineItem['type'], active: boolean) => {
   ...
   switch (type) {
      case 'project':
         return <BriefcaseIcon className={base} />;
      case 'training':
         return <GraduationCapIcon className={base} />;
      case 'achievement':
         return <TrophyIcon className={base} />;
      case 'milestone':
         return <TargetIcon className={base} />;
      default:
